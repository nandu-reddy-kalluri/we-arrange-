...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const item = SUBSECTION_ITEMS.find(i => i.categorySlug === "photography" && i.slug === slug);
  if (!item) return {};

  return {
    title: `${item.title} - Photography & Cinematography | YouMarriageWeArrange`,
    description: item.shortDescription,
    alternates: {
      canonical: `https://www.youmarriagewearrange.com/inspiration/photography/${item.slug}`
    },
    openGraph: {
      title: item.title,
      description: item.shortDescription,
...
export default async function PhotographyDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const relatedItems = SUBSECTION_ITEMS.filter(i => i.categorySlug === "photography" && i.slug !== item.slug).slice(0, 4);
